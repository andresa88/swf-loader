...
module.exports = function(content) {
  ...
  if (options.emitFile === undefined || options.emitFile) {
    ...
    try {
      // openfljs cannot process jpeg or png assets in the swf, so I switched to the openfl
      // binary which comes with the haxe openfl library. Install that via: haxelib install openfl
      execSync ("openfl process " + this.resourcePath + " " + tempDirectory.name);
    } catch (error) {
      var message = error.output.toString();
      if (message.indexOf('is not recognized as an internal or external command') > -1) {
        throw new Error('openfl command not found! Please install the openfl haxe library via:\nhaxelib install openfl\nhaxelib run openfl setup');
      }
      throw error;
    }
